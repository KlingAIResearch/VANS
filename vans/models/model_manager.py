...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device, mllm_query_dim=None):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        with init_weights_on_device():
            if mllm_query_dim is not None:
                extra_kwargs['mllm_query_dim'] = mllm_query_dim
...
class ModelDetectorFromSingleFile:
    ...
    def load(self, file_path="", state_dict={}, device="cuda", torch_dtype=torch.float16, mllm_query_dim=None, **kwargs):
        if len(state_dict) == 0:
            state_dict = load_state_dict(file_path)
        ...
        if keys_hash_with_shape in self.keys_hash_with_shape_dict:
            ...
            if mllm_query_dim is not None:
                loaded_model_names, loaded_models = load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device, mllm_query_dim=mllm_query_dim)
            else:
...
class ModelManager:
    ...
    def load_model(self, file_path, model_names=None, device=None, torch_dtype=None, mllm_query_dim=None):
