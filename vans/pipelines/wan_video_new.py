...
from ..models_mllm.agent_mllm import MLLMAgent
import re
...
class WanVideoPipeline(BasePipeline):
    ...
    def training_loss(self, **inputs):
        ...
        if self.mllm is not None:
            LM_loss, query_emb, query_attention_mask = self.mllm(inputs)
            return {"LM Loss": LM_loss}
            #query_emb = self.dit.mllm_query_embedding(query_emb)
            #inputs["query_emb"] = query_emb
        else:
            LM_loss = 0
            query_emb = None
        ...
        if inputs['ref_VAE_latents'] is not None:
            total_frames = inputs["ref_VAE_latents"].shape[2]
            indices = torch.linspace(0, total_frames - 1, 6).long()
            sampled_frames = inputs["ref_VAE_latents"][:, :, indices, :, :]
            inputs["latents"] = torch.cat([sampled_frames, inputs['latents']], dim=2)
        noise_pred = self.model_fn(**inputs, timestep=timestep)

        if inputs['ref_VAE_latents'] is not None:
            noise_pred = noise_pred[:, :, sampled_frames.shape[2]:, :, :]

        loss = torch.nn.functional.mse_loss(noise_pred.float(), training_target.float())
        loss = loss * self.scheduler.training_weight(timestep)
        return {"FM Loss": loss, "LM Loss": LM_loss}
    ...
    def from_pretrained(
        ...
        if use_mllm:
            pipe.mllm = MLLMAgent(mllm_pretrained_path=mllm_pretrained_path).to(device)
        ...
        for model_config in model_configs:
            ...
            if 'diffusion_pytorch_model.safetensors' in model_config.path:
                model_manager.load_model(
                    model_config.path,
                    device=device,
                    torch_dtype=model_config.offload_dtype or torch_dtype,
                    mllm_query_dim=mllm_query_dim,
                )
            else:
                model_manager.load_model(
                    model_config.path,
        ...
        if trained_ckpt_path is not None:
            ...
            if trained_ckpt_path.endswith('.safetensors'):
                ...
                state_dict = {}
                with safe_open(trained_ckpt_path, framework="pt", device="cpu") as f:
                    for key in f.keys():
                        state_dict[key] = f.get_tensor(key)
            else:
                state_dict = torch.load(trained_ckpt_path, map_location="cpu")
            
            pipe_state_dict = pipe.state_dict()
            pipe_keys = set(pipe_state_dict.keys())
            state_dict = {
                "dit." + key if ".mllm." not in key else key.replace("pipe.",""): value 
                for key, value in state_dict.items()
            }
            checkpoint_keys = set(state_dict.keys())

            matching_keys = pipe_keys & checkpoint_keys
            noload_keys = pipe_keys - checkpoint_keys
            if matching_keys:
                filtered_state_dict = {k: state_dict[k] for k in matching_keys}
                pipe.load_state_dict(filtered_state_dict, strict=False)
                print(f"[Loaded]: {matching_keys}")

        if mllm_trained_ckpt_path is not None:
            print(f"Loading trained checkpoint from: {mllm_trained_ckpt_path}")
            
            if mllm_trained_ckpt_path.endswith('.safetensors'):
                from safetensors import safe_open
                state_dict = {}
                with safe_open(mllm_trained_ckpt_path, framework="pt", device="cpu") as f:
                    for key in f.keys():
                        state_dict[key] = f.get_tensor(key)
            else:
                state_dict = torch.load(mllm_trained_ckpt_path, map_location="cpu")
            ...
            if matching_keys:
                ...
                print(f"[Loaded MLLM]: {matching_keys}")
    ...
    def __call__(
        ...
        ref_VAE_video: Optional[list[Image.Image]] = None,
        ...
        if self.mllm is not None:
            mllm_generation_captions, query_emb = self.mllm.generate_with_query(instructions=instructions, prompt=prompt,video_paths=input_video_path)
            query_emb = None # self.dit.mllm_query_embedding(query_emb)
            prompt = extract_caption(mllm_generation_captions[0])
            #breakpoint()
            #return mllm_generation_captions, mllm_generation_captions
        else:
            mllm_generation_captions = None
            query_emb = None
        ...
        if query_emb is not None:
            inputs_shared["query_emb"] = query_emb        
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            if inputs_shared["ref_VAE_latents"] is not None:
                #breakpoint()
                total_frames = inputs_shared["ref_VAE_latents"].shape[2]
                indices = torch.linspace(0, total_frames - 1, 6).long()
                sampled_frames = inputs_shared["ref_VAE_latents"][:, :, indices, :, :]
                inputs_shared["latents"] = torch.cat([sampled_frames, inputs_shared['latents']], dim=2)
                #inputs_shared["latents"] = torch.cat([inputs_shared["ref_VAE_latents"], inputs_shared['latents']], dim=2)
            ...
            if inputs_shared["ref_VAE_latents"] is not None:
                inputs_shared["latents"] = inputs_shared["latents"][:, :, sampled_frames.shape[2]:, :, :]
                #inputs_shared["latents"] = inputs_shared["latents"][:,:,inputs_shared['ref_VAE_latents'].shape[2]:,:,:]
        ...
        if self.mllm is not None:
            return mllm_generation_captions[0], video
        return video
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    def __init__(self):
        super().__init__(
            input_params=("input_video", "ref_VAE_video", "noise", "tiled", "tile_size", "tile_stride", "vace_reference_image"),
            onload_model_names=("vae",)
        )

    def process(self, pipe: WanVideoPipeline, input_video, ref_VAE_video, noise, tiled, tile_size, tile_stride, vace_reference_image):

        if ref_VAE_video is not None:
            ref_VAE_video = pipe.preprocess_video(ref_VAE_video)
            ref_VAE_latents = pipe.vae.encode(ref_VAE_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device)
        else:
            ref_VAE_latents = None 

        if input_video is None:
            return {"latents": noise, "ref_VAE_latents": ref_VAE_latents}
        ...
        if pipe.scheduler.training:
            return {"latents": noise, "input_latents": input_latents, "ref_VAE_latents":ref_VAE_latents}
        else:
            latents = pipe.scheduler.add_noise(input_latents, noise, timestep=pipe.scheduler.timesteps[0])
            return {"latents": latents, "ref_VAE_latents": ref_VAE_latents}
    ...
    if tea_cache_update:
        ...
        def create_custom_forward(module):
            def custom_forward(*inputs, **kwargs):
                all_args = list(inputs)
                if kwargs:
                    return module(*all_args, **kwargs)
                else:
                    return module(*all_args)
        ...
        for block_id, block in enumerate(dit.blocks):
            ...
            if use_gradient_checkpointing_offload:
                ...
                x = block(x, context, t_mod, freqs, query_emb=query_emb)
